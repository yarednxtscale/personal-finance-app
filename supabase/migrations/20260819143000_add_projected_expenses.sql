...
create policy "projected_expenses_insert_own" on public.projected_expenses for insert to authenticated with check ((select auth.uid()) = user_id);
create policy "projected_expenses_update_own" on public.projected_expenses for update to authenticated using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
